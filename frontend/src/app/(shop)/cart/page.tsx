...
export default function CartPage() {
  // ✅ Obtenemos el token e isAuthenticated para el control de hidratación
  ...
  // ✅ LOGICA ANTI-FLICKER:
  // Mostramos el loading si el hook dice que está cargando 
  // O si estamos logueados pero el token aún se está recuperando (hidratación)
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 overflow-x-hidden flex flex-col">
      <main className="flex-grow w-[90%] max-w-[1400px] mx-auto pt-32 pb-12 relative">
        ...
        {cartItems.length === 0 ? (
          ...
          /* --- CONTENIDO CARRITO (Sin cambios en tu lógica B2B) --- */
          <>
            ...
            <div className="flex flex-col lg:flex-row gap-8 md:gap-10 items-start">
              ...
              <div className="w-full lg:w-[400px] flex-shrink-0">
                <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 sticky top-28">
                  <h2 className="text-lg md:text-xl font-black text-slate-900 mb-6 flex items-center gap-2">Resumen <span className="text-slate-300 font-light">|</span> Financiero</h2>
                  ...
                  <div className="space-y-4">
                      <Link href="/checkout" className="group w-full bg-slate-900 text-white py-3.5 md:py-4 rounded-xl md:rounded-2xl font-bold text-base md:text-lg hover:bg-blue-600 transition-all shadow-xl shadow-blue-900/10 flex items-center justify-center gap-3">
                        Proceder al Pago <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform text-white/50 group-hover:text-white"/>
                      </Link>
                      <Link href="/products" className="w-full block text-center py-2 md:py-3 text-xs font-black text-slate-400 hover:text-blue-600 uppercase tracking-widest transition-colors">Continuar Comprando</Link>
                  </div>
                  <div className="mt-6 md:mt-8 bg-blue-50/50 p-4 rounded-xl md:rounded-2xl border border-blue-100 flex items-start gap-3">
                    ...
                    <p className="text-[10px] md:text-[11px] text-blue-700 leading-relaxed font-medium"><strong>Nota B2B:</strong> Los costos finales de envío y la facturación fiscal se procesarán en el siguiente paso de checkout seguro.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {isClearModalOpen && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 max-w-sm w-full shadow-2xl text-center animate-in zoom-in-95 duration-200 ring-1 ring-black/5">
              ...
              <p className="text-slate-500 font-medium mb-6 md:mb-8 leading-relaxed text-sm md:text-base">Esta acción eliminará todos los productos seleccionados de tu orden actual. No podrás deshacer esta acción.</p>
              <div className="flex gap-3">
                <button onClick={() => setIsClearModalOpen(false)} className="flex-1 py-3 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors text-sm">Cancelar</button>
                <button onClick={confirmClearCart} className="flex-1 py-3 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 transition-colors shadow-lg shadow-red-500/20 text-sm flex items-center justify-center gap-2"><Trash2 size={16} /> Vaciar</button>
              </div>
            </div>
          </div>
        )}
