      ...
      <main className="flex-grow w-[90%] max-w-[1400px] mx-auto pt-32 pb-12 relative">
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 md:py-20 text-center animate-in fade-in zoom-in-95 duration-500">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 mb-3 md:mb-4 tracking-tight">Your cart is empty</h1>
            <p className="text-slate-500 mb-8 md:mb-10 max-w-md text-base md:text-lg leading-relaxed font-medium px-4">
              It seems you haven't added any medical supplies to your order yet. Explore our catalog to find current lots.
            </p>
            <Link 
              href="/products" 
            ...
            >
              Explore Catalog <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform md:w-5 md:h-5" />
            </Link>
          </div>
        ) : (
          <>
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 gap-4">
               ...
               <span className="bg-blue-100 text-blue-700 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider w-fit">
                 {summary.totalItems} Items added
               </span>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 md:gap-10 items-start">
              ...
              <div className="flex-1 w-full space-y-4 md:space-y-6">
                {cartItems.map((item) => (
                  <div key={item.cart_item_id} className="group bg-white p-4 md:p-6 rounded-2xl md:rounded-[2.5rem] border border-white shadow-sm hover:shadow-xl hover:border-blue-100 transition-all duration-300">
                    <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-3">
                          <div className="pr-2">
                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-1">{item.manufacturer_name}</p>
                            <h3 className="font-bold text-slate-800 text-lg md:text-xl leading-tight hover:text-blue-700 transition-colors line-clamp-2">{item.product_name}</h3>
                          </div>
                          <button onClick={() => removeItem(item.cart_item_id)} className="text-slate-300 hover:text-red-500 p-2 hover:bg-red-50 rounded-xl transition-all flex-shrink-0">
                        ...
                        <div className="inline-flex flex-wrap items-center gap-x-4 gap-y-2 bg-slate-50 rounded-xl px-3 py-2 border border-slate-100 mb-4 md:mb-5 w-full sm:w-auto">
                          <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                            ...
                            Lot: <span className="font-mono font-bold text-slate-900">{item.lot_number}</span>
                          </div>
                        ...
                        <div className="flex flex-wrap items-end justify-between gap-4">
                          ...
                          <div className="text-right">
                             ...
                             <div className="flex items-baseline justify-end gap-2">
                               ...
                               <p className="text-xl md:text-2xl font-black text-slate-900 leading-none">{formatCurrency(parseFloat(item.unit_price) * item.cart_quantity)}</p>
                             </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex justify-end pt-2 md:pt-4">
                  <button onClick={() => setIsClearModalOpen(true)} className="text-red-500 text-xs font-black uppercase tracking-widest hover:text-red-700 flex items-center gap-2 px-4 py-2 hover:bg-red-50 rounded-lg transition-all">
              ...
              <div className="w-full lg:w-[400px] flex-shrink-0">
                <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 sticky top-28">
                  <h2 className="text-lg md:text-xl font-black text-slate-900 mb-6 flex items-center gap-2">Financial <span className="text-slate-300 font-light">|</span> Summary</h2>
                  <div className="space-y-4 mb-8">
                    ...
                    <div className="flex justify-between text-sm font-medium text-slate-500">
                      ...
                      <span className="text-emerald-600 font-bold text-[10px] md:text-xs bg-emerald-50 px-2 py-0.5 rounded">CALCULATE AT CHECKOUT</span>
                    </div>
                    <div className="flex justify-between text-sm font-medium text-slate-500">
                      <span>Taxes (VAT)</span>
                  ...
                  <div className="flex justify-between items-end py-6 border-t border-dashed border-slate-200 mb-6">
                    <span className="text-xs md:text-sm font-bold text-slate-500 uppercase tracking-widest">Estimated Total</span>
                    <span className="text-2xl md:text-3xl font-black text-slate-900 leading-none">{formatCurrency(summary.subtotal)}</span>
                  </div>
                  <div className="space-y-4">
                      <Link href="/checkout" className="group w-full bg-slate-900 text-white py-3.5 md:py-4 rounded-xl md:rounded-2xl font-bold text-base md:text-lg hover:bg-blue-600 transition-all shadow-xl shadow-blue-900/10 flex items-center justify-center gap-3">
                        Proceed to Checkout <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform text-white/50 group-hover:text-white"/>
                      </Link>
                      <Link href="/products" className="w-full block text-center py-2 md:py-3 text-xs font-black text-slate-400 hover:text-blue-600 uppercase tracking-widest transition-colors">Continue Shopping</Link>
                  </div>
                  <div className="mt-6 md:mt-8 bg-blue-50/50 p-4 rounded-xl md:rounded-2xl border border-blue-100 flex items-start gap-3">
                    ...
                    <p className="text-[10px] md:text-[11px] text-blue-700 leading-relaxed font-medium"><strong>B2B Note:</strong> Final shipping costs and tax invoicing will be processed in the next secure checkout step.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {isClearModalOpen && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity animate-in fade-in duration-200" onClick={() => setIsClearModalOpen(false)}></div>
            <div className="relative bg-white rounded-2xl md:rounded-[2.5rem] p-6 md:p-8 max-w-sm w-full shadow-2xl text-center animate-in zoom-in-95 duration-200 ring-1 ring-black/5">
              ...
              <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-2 md:mb-3">Clear Cart?</h3>
              <p className="text-slate-500 font-medium mb-6 md:mb-8 leading-relaxed text-sm md:text-base">This action will remove all selected products from your current order. This action cannot be undone.</p>
              <div className="flex gap-3">
                <button onClick={() => setIsClearModalOpen(false)} className="flex-1 py-3 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors text-sm">Cancel</button>
                <button onClick={confirmClearCart} className="flex-1 py-3 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 transition-colors shadow-lg shadow-red-500/20 text-sm flex items-center justify-center gap-2"><Trash2 size={16} /> Clear</button>
              </div>
            </div>
          </div>
        )}
