...
export default function NotificationsPage() {
  ...
  // --- VISUAL HELPERS (Updated for B2B) ---
  const getIcon = (type: string, subject: string) => {
    if (subject.includes('Proposal')) return <DollarSign size={24} />;
    if (subject.includes('Shipped')) return <Truck size={24} />;
  ...
  const getTheme = (type: string, message: string, subject: string) => {
    ...
    if (subject.includes('Cancelled') || message.includes('Rejected')) 
      return 'bg-red-50 text-red-600 border-l-4 border-red-500';
    
    if (subject.includes('Proposal') || message.includes('calculated shipping')) 
      return 'bg-sky-50 text-sky-700 border-l-4 border-sky-500 shadow-sky-100'; // Blue for ready quote
    
    if (subject.includes('Stock Approved') || subject.includes('Shipped')) 
  ...
  const getActionLink = (n: ClientNotification) => {
    // Always redirect to the corresponding list to open the modal
  ...
  const getActionLabel = (n: ClientNotification) => {
    ...
    if (subject.includes('Proposal')) return 'Review and Pay';
    if (subject.includes('Stock Approved')) return 'Upload Payment';
    if (subject.includes('Shipped')) return 'View Tracking';
    return 'View Details';
  };
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">
              Updates on your requests and shipments.
            </p>
          </div>
          ...
          {!isLoading && notifications.length > 0 && (
            <div className="bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-600">{notifications.length} New</span>
            </div>
          )}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-slate-400 text-sm">We will notify you when there are updates.</p>
          </div>
        )}
