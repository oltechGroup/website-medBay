          ...
          <div className="flex items-center gap-3 md:gap-4 flex-1 md:flex-none">
            ...
            {variant !== 'dashboard' && (
              <Link href="/" className="flex items-center gap-2 group flex-shrink-0 mr-auto md:mr-0 z-50">
                <img src="/icons/logomed.png" alt="Icon" className="w-9 h-9 md:w-10 md:h-10 rounded-lg transition-transform group-hover:scale-105" />
                {/* BRAND NAME AS IMAGE TO PREVENT AUTO-TRANSLATE */}
                <img 
                  src="/icons/MedBay.png" 
                  alt="MedBay" 
              ...
              {variant === 'catalog' ? (
                ...
                <nav className="flex items-center gap-8">
                  {[
                    ...
                    { label: 'Features', path: '/Characteristics' },
                ...
                {isNotifOpen && (
                  <div className="absolute right-[-60px] md:right-0 top-full mt-4 w-[90vw] md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                      <h4 className="text-xs font-black uppercase text-slate-500 tracking-wider">{isStaff ? 'Activity Center' : 'My Notifications'}</h4>
                      {activeUnreadCount > 0 && <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{activeUnreadCount} new</span>}
                    </div>
                    <div className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                      {activeNotifications.length === 0 ? (
                        <div className="p-10 text-center flex flex-col items-center">
                          ...
                          <p className="text-slate-400 text-sm font-medium">You are all caught up</p>
                        </div>
                      ) : (
                          ...
                          <div key={n.id} onClick={() => handleNotificationClick(n)} className="relative p-4 border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors group">
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-500 mt-0.5 truncate">{isStaff ? (n.sender_name || 'Unknown') : 'MedBay System'}</p>
                    ...
                    <Link href={isStaff ? "/dashboard" : "/notifications"} className="block p-3 text-center text-xs font-bold text-blue-600 hover:bg-blue-50 transition-colors border-t border-slate-100">View Full History</Link>
                  </div>
                )}
              </div>
            )}
            ...
            <div className={variant === 'dashboard' ? 'block' : 'hidden sm:block'}>
              {mounted && isAuthenticated && user ? (
                    ...
                    <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="p-2 space-y-1">
                        {isStaff ? (
                          ...
                          <>
                            <Link href="/profile" className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-xl transition-colors"><Settings size={16} /> My Profile</Link>
                            <Link href="/orders" className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-xl transition-colors"><Package size={16} /> My Orders</Link>
                            <Link href="/quotes" className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-xl transition-colors"><MessageSquareQuote size={16} /> My Quotes</Link>
                        ...
                        <button onClick={() => logout()} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-bold text-red-600 hover:bg-red-50 rounded-xl transition-colors"><LogOut size={16} /> Logout</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Link href="/login" className="text-sm font-bold text-slate-600 hover:text-blue-600 px-3 py-2 transition-colors">Login</Link>
                  <Link href="/register" className="bg-slate-900 text-white text-sm font-bold px-4 py-2 rounded-xl hover:bg-blue-600 transition-all shadow-md">Register</Link>
                </div>
              )}
            </div>
        ...
        <div className="lg:hidden fixed inset-0 z-[990] bg-white animate-in slide-in-from-top-10 fade-in duration-200 flex flex-col pt-[72px]">
          <div className="flex-1 overflow-y-auto px-6 pb-20 custom-scrollbar">
            ...
            <nav className="flex flex-col gap-3 mb-8">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 pl-1">Main Menu</p>
              {[
                { label: 'Product Catalog', path: '/products', icon: <Package size={20}/>, color: 'text-blue-500' },
                { label: 'Features', path: '/Characteristics', icon: <ShieldCheck size={20}/>, color: 'text-emerald-500' },
                { label: 'About Us', path: '/About', icon: <Building2 size={20}/>, color: 'text-indigo-500' },
                { label: 'Contact & Support', path: '/Contact', icon: <MessageSquareQuote size={20}/>, color: 'text-amber-500' }
              ].map(link => (
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4 relative z-10">
                    {isStaff ? (
                      ...
                      <>
                        ...
                        <Link href="/orders" onClick={() => setIsMobileMenuOpen(false)} className="bg-white/10 border border-white/5 py-3 rounded-xl text-center font-medium hover:bg-white/20 transition-colors">Orders</Link>
                        <Link href="/quotes" onClick={() => setIsMobileMenuOpen(false)} className="bg-white/10 border border-white/5 py-3 rounded-xl text-center font-medium hover:bg-white/20 transition-colors">Quotes</Link>
                        <Link href="/wishlist" onClick={() => setIsMobileMenuOpen(false)} className="bg-white/10 border border-white/5 py-3 rounded-xl text-center font-medium hover:bg-white/20 transition-colors">Favorites</Link>
                      </>
                    )}
                  </div>
                  <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="w-full flex items-center justify-center gap-2 py-3 text-red-300 hover:text-white font-medium transition-colors relative z-10"><LogOut size={18} /> Logout</button>
                </div>
              ) : (
                <div className="flex flex-col gap-4">
                  <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="w-full py-4 text-center font-bold text-slate-700 bg-white border-2 border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200 transition-all">Login</Link>
                  <Link href="/register" onClick={() => setIsMobileMenuOpen(false)} className="w-full py-4 text-center font-bold text-white bg-blue-600 rounded-2xl shadow-xl shadow-blue-600/20 hover:bg-blue-700 transition-all flex items-center justify-center gap-2">Create Free Account <ArrowRight size={18} /></Link>
                </div>
              )}
            </div>
          </div>
