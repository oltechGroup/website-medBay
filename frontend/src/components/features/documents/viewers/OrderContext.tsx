...
export const OrderContext = ({ orderId }: { orderId: string }) => {
  ...
  if (!order) return <div className="p-4 text-slate-400">No se encontró información de la orden.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Orden Relacionada</h4>
      ...
      <div>
        <h4 className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-3">
          <Package size={16} className="text-blue-500"/> Productos ({items.length})
        </h4>
          ...
          {items.map((item: any) => (
            <div key={item.id} className="flex justify-between items-center text-sm p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-400">Cant: {item.quantity} | SKU: {item.global_sku}</p>
              </div>
