...
export const UserContext = ({ userId }: { userId: string }) => {
  ...
  // Buscamos la dirección fiscal
  const fiscalAddress = user.addresses?.find((a: any) => a.is_fiscal) || user.addresses?.[0];

  // Helper para etiquetas de rol bonitas
  const getRoleUI = (role: string) => {
    switch (role) {
      case 'business_verified': 
        return { label: 'Empresarial', className: 'bg-indigo-50 text-indigo-700 border-indigo-200' };
      case 'medical_professional': 
        return { label: 'Profesional', className: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
      case 'sales_agent':
        return { label: 'Vendedor', className: 'bg-purple-50 text-purple-700 border-purple-200' };
      case 'admin':
        return { label: 'Administrador', className: 'bg-slate-800 text-white border-slate-900' };
      default: 
        return { label: 'Consumidor', className: 'bg-slate-50 text-slate-600 border-slate-200' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="pt-2 border-t border-slate-50">
             <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Nivel Solicitado</p>
      ...
      {fiscalAddress && (
        <div>
          <h4 className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-3">
            <MapPin size={16} className="text-blue-500"/> Dirección Fiscal Completa
          </h4>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 text-sm text-slate-600 leading-relaxed shadow-sm space-y-3">
            ...
            <div>
              <p className="font-bold text-slate-900 text-base">
                ...
                {fiscalAddress.suite_number && <span className="text-slate-500 font-normal ml-1">Int. {fiscalAddress.suite_number}</span>}
              </p>
            ...
            <div className="flex gap-2 text-xs font-bold uppercase tracking-wide text-slate-400">
              ...
              <span>•</span>
              <span>CP: {fiscalAddress.postal_code}</span>
            </div>
            ...
            {/* Referencias Extra (Solo si existen) */}
              ...
              <div className="pt-3 border-t border-slate-50 mt-2">
                {fiscalAddress.between_streets && (
                  <div className="flex gap-2 mb-1">
                    ...
                    <p className="text-xs text-slate-500"><span className="font-bold">Entre calles:</span> {fiscalAddress.between_streets}</p>
                  </div>
                )}
                {fiscalAddress.reference_point && (
                  <div className="flex gap-2">
                    <Info size={14} className="text-blue-400 shrink-0 mt-0.5"/>
                    <p className="text-xs text-slate-500"><span className="font-bold">Referencia:</span> {fiscalAddress.reference_point}</p>
                  </div>
                )}
              </div>
            )}
