...
interface QuoteDetailsProps {
  details: any; // Viene de content.product_details
  message?: string;
}

export default function QuoteDetails({ details, message }: QuoteDetailsProps) {
  // Mapeo de colores según el tipo de solicitud para identificación rápida visual
  const getTypeBadgeColor = (type: string) => {
    if (type?.includes('Caducado')) return 'bg-red-100 text-red-700 border-red-200';
    if (type?.includes('Fecha Corta')) return 'bg-amber-100 text-amber-700 border-amber-200';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
          <ShoppingBag size={14} /> Producto Solicitado
        </h4>
      ...
      {/* 2. Detalles de la Solicitud (Grid) */}
      <div className="grid grid-cols-2 gap-4">
        {/* Cantidad */}
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
          <p className="text-xs text-slate-500 font-bold uppercase mb-1 flex items-center gap-1">
            <Package size={14} /> Cantidad
          </p>
          <p className="text-2xl font-black text-slate-800">
            {details.quantity} <span className="text-sm font-medium text-slate-400">pzas</span>
          </p>
        </div>
        ...
        <div className={`p-4 rounded-xl border ${getTypeBadgeColor(details.type)}`}>
          <p className="text-xs font-bold uppercase mb-1 flex items-center gap-1 opacity-80">
            <Calendar size={14} /> Tipo Requerido
          </p>
      ...
      {/* 3. Mensaje / Notas del Cliente */}
      {message && (
        <div className="bg-white p-5 rounded-2xl border border-slate-200">
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
            <AlertCircle size={14} /> Notas del Cliente
          </h4>
