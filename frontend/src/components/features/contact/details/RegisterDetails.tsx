...
export default function RegisterDetails({ details }: RegisterDetailsProps) {
  ...
  // --- BASE URL LOGIC PRESERVED ---
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api.medbaysupply.com';
  
  // Logic to ensure we point to the server root for static files
    ...
    <div className="space-y-6">
      ...
      <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100">
        <h4 className="text-xs font-bold text-emerald-700 uppercase tracking-widest mb-4 flex items-center gap-2">
          <UserCheck size={14} /> Requested Profile
        </h4>
        ...
        <div className="flex items-center gap-3">
           ...
           <div>
              ...
              <p className="text-xs text-slate-500 font-bold mt-1">Requires manual validation</p>
           </div>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
          <Building2 size={14} /> Tax Information
        </h4>
        
        <div className="space-y-3 text-sm">
           <div className="flex justify-between border-b border-slate-100 pb-2">
              <span className="text-slate-500 font-medium">Company / Legal Entity:</span>
              <span className="font-bold text-slate-800">{details.company || 'N/A'}</span>
           </div>
           <div className="flex justify-between pb-2">
              <span className="text-slate-500 font-medium flex items-center gap-1"><Hash size={12}/> Tax ID:</span>
      ...
      {hasFile && (
        <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100">
          <h4 className="text-xs font-bold text-blue-700 uppercase tracking-widest mb-4 flex items-center gap-2">
            <FileText size={14} /> Attached Documentation
          </h4>
          ...
          <p className="text-[10px] text-blue-600/60 mt-2 px-1">
             * This document is required to validate the requested verification level.
          </p>
        </div>
      )}
