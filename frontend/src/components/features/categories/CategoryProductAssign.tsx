...
export const CategoryProductAssign = ({ categories, onAssignComplete }: CategoryProductAssignProps) => {
  ...
  // Filtrar categorías y productos según búsqueda
  ...
  // Estadísticas reales - CORREGIDO: sin products_count
  ...
  return (
    <div className="space-y-6">
      {/* 📊 Contadores con diseño de cards bonitas (SOLO 3 TARJETAS) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">Sin Productos</p>
      ...
      {/* 🔮 Barra de Progreso Mejorada (SOLO UNA) */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Progreso de Categorización</h3>
            <p className="text-gray-600 mt-1">
              {categoriesWithProducts.length} de {categories.length} categorías tienen productos asignados
            </p>
          </div>
      ...
      {(selectedCategories.length > 0 || selectedProducts.length > 0) && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            ...
            >
              {isBatchAssigning ? '🔄 Asignando...' : '✅ Asignar Productos'}
            </button>
          </div>
        </div>
      )}
      ...
      <div className={`relative top-20 mx-auto p-5 border shadow-lg rounded-md bg-white ${showDetails ? 'w-11/12 max-w-4xl' : 'w-96'}`}>
        <div className="mt-3 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900">
            {showDetails ? 'Detalles de la Asignación' : '¿Asignar productos?'}
          </h3>
          
          <div className="mt-2 px-7 py-3">
            <p className="text-sm text-gray-500">
              Estás a punto de asignar <strong>{selectedProducts.length} producto(s)</strong> a{' '}
              <strong>{selectedCategories.length} categoría(s)</strong>.
            </p>

            {/* VISTA NORMAL (como productos) */}
            {!showDetails && (
              <>
